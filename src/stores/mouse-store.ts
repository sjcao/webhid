...
async function readWithAck(command: Uint8Array, type: ParamType, buttonId?: ButtonId) {
  for (let attempt = 0; attempt < 2; attempt += 1) {
    const { promise, cancel } = waitForResponse(type, buttonId);
...
export const useMouseStore = create<MouseState>((set, get) => ({
  ...
  refreshInitialState: async () => {
    ...
    if (previewMode) {
      ...
      for (const buttonId of ALL_BUTTON_IDS) responsive = (await readWithAck(MouseCommands.readButton(buttonId), ParamType.Button, buttonId)) && responsive;
      set({ deviceUnresponsive: !responsive });
    }
  },
  selectProfile: async (profile) => { set({ activeProfile: profile }); try { await sendOrPreview(MouseCommands.switchProfile(profile)); } catch (error) { set(commandFailure(error)); await resendReads([MouseCommands.readProfile()]); } },
  updateDpi: async (dpi) => { set({ dpi }); try { await sendOrPreview(MouseCommands.setDpi(dpi)); } catch (error) { set(commandFailure(error)); await resendReads([MouseCommands.readDpi()]); } },
  readButton: async (buttonId) => { try { await sendOrPreview(MouseCommands.readButton(buttonId)); } catch (error) { set(commandFailure(error)); } },
  setButtonMapping: async (payload) => { set((state) => ({ buttonConfigs: { ...state.buttonConfigs, [payload.buttonId]: payload } })); try { await sendOrPreview(MouseCommands.setButtonMapping(payload)); } catch (error) { set(commandFailure(error)); await resendReads([MouseCommands.readButton(payload.buttonId)]); } },
  bindComboToButton: async (buttonId, values) => { const payload = { buttonId, functionType: KeyFunctionType.ComboKey, index: 0, values }; set((state) => ({ buttonConfigs: { ...state.buttonConfigs, [buttonId]: payload } })); try { await sendOrPreview(MouseCommands.setButtonMapping(payload)); } catch (error) { set(commandFailure(error)); await resendReads([MouseCommands.readButton(buttonId)]); } },
  bindMacroToButton: async (buttonId, macro) => {
    ...
    const frames = macro.actions.map((action, index) => MouseCommands.setButtonMacro({ buttonId, macroId: slot, repeatType, macroButtonType: macroButtonTypeForAction(action), delayMs: getMacroDelay(index, macro.actions), values: action.direction === 'up' ? [0x00] : normalizeMacroValue(action.keyCode) }));
    const terminator = MouseCommands.setButtonMacro({ buttonId, macroId: slot, repeatType, macroButtonType: MacroButtonType.KeyUp, delayMs: 0, values: [0x00] });
    try { await sendBatchOrPreview([...frames, terminator]); } catch (error) {
  ...
  resetButtons: async () => { set({ buttonConfigs: {} }); try { await sendOrPreview(MouseCommands.resetButtons()); } catch (error) { set(commandFailure(error)); await resendReads(ALL_BUTTON_IDS.map((buttonId) => MouseCommands.readButton(buttonId))); } },
  resetAll: async () => { set({ activeProfile: 0, dpi: 1600, buttonConfigs: {} }); try { await sendOrPreview(MouseCommands.resetAll()); } catch (error) { set(commandFailure(error)); await resendReads([MouseCommands.readProfile(), MouseCommands.readDpi(), ...ALL_BUTTON_IDS.map((buttonId) => MouseCommands.readButton(buttonId))]); return; } await get().refreshInitialState(); },
}));
...
function normalizeMacroValue(values: number[]) { return values.length === 2 && values[1] === 0x00 ? [values[0]] : values; }
